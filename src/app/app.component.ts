...
export class AppComponent {
  ...
  books: Book[] = this.allBooks;
  cart: CartItem[] = [];
  searchText: string = "";
  total: number = 0;

  addToCart($event: Book) {
    let idx = this.cart.findIndex(cartItem => cartItem.book == $event);

    if (idx === -1) {
      this.cart.push({ book: $event, quantity: 1 });
    } else {
      this.cart[idx].quantity += 1;
    }
    this.total += $event.price.value;
  }
  ...
  filterBooksByTitle(searchText: string) {
    if(searchText.length == 0) {
      this.books = this.allBooks;
    } else {
      this.books = this.allBooks.filter(book => {
        return book.title.toLocaleLowerCase().includes(searchText.toLocaleLowerCase());
      })
    }
